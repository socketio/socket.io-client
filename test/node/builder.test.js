...
module.exports = {
  ...
  'node if': function () {
    ...
    builder({ custom: [custom], minify: false }, function (err, result) {
      ...
      result.should.not.include(custom);
      result.should.not.include('// if node');
      result.should.not.include('// end node');
      result.should.not.include('"world"');

      result.should.include('var pew = "pew"');
    });
  },
